...
async def process_queries(request: QueryRequest):
    try:
        ...
        if not doc_id:
            ...
            text = processor.extract_text(request.documents)
        ...
        for i, question in enumerate(request.questions):
            ...
            answer = decision_engine.process_queries([question], doc_id=doc_id)  # No need to pass doc_name
            response = answer[0] if answer else "No relevant clauses found."
            answers.append(response)
            
            # Log query and response
            logger_manager.log_query(request.documents, doc_id, question, response)
